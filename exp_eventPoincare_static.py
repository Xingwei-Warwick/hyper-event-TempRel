...
from event_poincare_emb import EventTempRel_poincare_static
...
class experiment:
    def __init__(self, device, embedding_dim, trainset, testset, output_labels,
                modelPath, args=None):
        self.model = EventTempRel_poincare_static(device=device, num_neg=args.neg,
    ...
    def trainHelper(self,trainset,testset,max_epoch):
        ...
        for epoch in range(max_epoch):
            ...
            for i, temprel in enumerate(trainset):
                ...
                if self.context == 'elmo':
                    e_pair = temprel.event_ix   # [event1_idx, event2_idx]
                else:
                ...
                for j in range(self.model.num_neg):
                    ...
                    s_neg.append(trainset[relid].token)
                    if self.context == 'elmo':
                        if is_source:
                            sneg_epos.append(trainset[relid].event_ix[0])
                        else:
                            sneg_epos.append(trainset[relid].event_ix[1])
                    else:
                        ...
                        neg_embeds = self.lm.get_encoded_context(relpair)
                        neg_emb_cache.append(neg_embeds)
                        current_batch['neg_sent_len'].append(neg_embeds.size(0))
                ...
                if self.context == 'elmo':
                    ...
                    for tk in s_neg:
                        if not self.lowerCase:
                            neg_embeds = self.emb_cache.retrieveEmbeddings(tokList=tk)
                        else:
                            neg_embeds = self.emb_cache.retrieveEmbeddings(tokList=[x.lower() for x in tk])
                        current_batch['neg_sent_len'].append(neg_embeds.size(0))
                        neg_emb_cache.append(neg_embeds)
                else:
                    relpair = temprel.docid+temprel.source+temprel.target
                ...
                current_batch['sneg_sen'].append(neg_emb_cache)
                ...
                if len(current_batch['sentence']) >= self.batch_size:
                    ...
                    for eve_pair in current_batch['events']:
                        tempmask = np.zeros((seq_max_len, 1))
                    ...
                    for negs in current_batch['sneg_sen']:
                        for neg in negs:
                            slen = neg.size(0)
                            if slen != neg_seq_max:
                                neg = nn.functional.pad(neg, pad=(0,0,0,neg_seq_max-slen))
                            neg_sen_vectors.append(neg)
                    ...
                    loss = self.model(s, u, v, neg_seq, mask_u_neg)
    ...
    def eval(self, eval_on_set, gen_output=False, is_test=False, thresh=None):
        ...
        for it,ex in enumerate(eval_on_set):
            ...
            if len(current_batch['sentence']) > self.batch_size or it == len(eval_on_set)-1:
                ...
                score = self.model.get_score(s, u, v)
                #print(score)
